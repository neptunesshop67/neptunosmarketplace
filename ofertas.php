...
$stmt = $pdo->query("SELECT p.*, c.nombre as categoria_nombre FROM productos p LEFT JOIN categorias c ON p.categoria_id = c.id WHERE p.descuento > 0 ORDER BY p.descuento DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_to_cart') {
    ...
    if ($productId > 0) {
        ...
        header('Location: ofertas.php?added=1');
        exit;
    }
}
$added = isset($_GET['added']);

$pageTitle = 'Ofertas Especiales | MercaShop';
...
<div class="space-y-6 bg-white p-6 rounded-md shadow-sm border border-slate-200 mb-6">
  <div class="border-b border-slate-200 pb-4 mb-4">
    <h2 class="text-2xl font-bold text-slate-900">
      Las mejores <span class="text-[#00a650]">Ofertas</span>
    </h2>
    <p class="text-sm text-slate-500 mt-1">Descubre productos con grandes descuentos y envío gratis.</p>
  </div>
  ...
  <?php if ($added): ?>
    <div class="rounded-md border border-[#00a650] bg-[#e6f7ee] px-4 py-3 text-[#00a650] font-semibold flex items-center gap-2 mb-4">
      ...
      ¡Agregaste una oferta a tu carrito!
    </div>
  <?php endif; ?>
  ...
  <?php else: ?>
    <div class="p-8 text-center text-slate-600">No hay ofertas disponibles por el momento.</div>
  <?php endif; ?>
</div>
