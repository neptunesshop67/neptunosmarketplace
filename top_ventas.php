...
$stmt = $pdo->query("SELECT p.*, c.nombre as categoria_nombre FROM productos p LEFT JOIN categorias c ON p.categoria_id = c.id ORDER BY p.stock ASC LIMIT 20");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_to_cart') {
    ...
    if ($productId > 0) {
        ...
        header('Location: top_ventas.php?added=1');
        exit;
    }
}
$added = isset($_GET['added']);

$pageTitle = 'Más Vendidos | MercaShop';
...
<div class="space-y-6 bg-white p-6 rounded-md shadow-sm border border-slate-200 mb-6">
  <div class="border-b border-slate-200 pb-4 mb-4 flex items-center justify-between">
    <div>
      <h2 class="text-2xl font-bold text-slate-900">
        Los más <span class="text-[#3483fa]">vendidos</span>
      </h2>
      <p class="text-sm text-slate-500 mt-1">Los productos favoritos de nuestra comunidad.</p>
    </div>
      ...
      <?php foreach ($productos as $product): ?>
        <article class="group bg-white rounded-md border border-slate-200 hover:shadow-lg transition-shadow overflow-hidden flex flex-col relative">
          <a href="product.php?id=<?= intval($product['id']) ?>" class="block aspect-square bg-slate-100 relative">
            ...
            <span class="absolute top-2 left-2 bg-[#ffa41c] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-sm uppercase tracking-wider">Más vendido</span>
          </a>
          ...
          <div class="p-4 flex flex-col flex-1 border-t border-slate-100">
            ...
            <div class="flex items-center mt-1 mb-2">
              <div class="flex text-[#ffa41c] text-sm">★★★★★</div>
              <span class="text-xs text-[#3483fa] ml-1">(<?= rand(500, 2000) ?>)</span>
            </div>
  ...
  <?php else: ?>
    <div class="p-8 text-center text-slate-600">No hay productos destacados por el momento.</div>
  <?php endif; ?>
</div>
